...
#ifndef SYMBOLICORE_SYMBOLIC_TEMPLATES_TPL_HPP
#define SYMBOLICORE_SYMBOLIC_TEMPLATES_TPL_HPP
...
template<class F, class J> inline auto _derivative_impl(Abs op, F const& f, J j) -> F {
    SYMBOLICORE_THROW(std::runtime_error,"derivative(abs(f))","Cannot take derivative of non-smooth function"); }
...
template<class F1, class F2, class J> inline auto _derivative_impl(Max, F1 const& f1, F2 const& f2, J j) -> decltype(max(f1,f2)){
    SYMBOLICORE_THROW(std::runtime_error,"derivative(max(f1,f2))","Cannot take derivative of non-smooth function."); }
template<class F1, class F2, class J> inline auto _derivative_impl(Min, F1 const& f1, F2 const& f2, J j) -> decltype(min(f1,f2)) {
    SYMBOLICORE_THROW(std::runtime_error,"derivative(min(f1,f2))","Cannot take derivative of non-smooth function."); }
